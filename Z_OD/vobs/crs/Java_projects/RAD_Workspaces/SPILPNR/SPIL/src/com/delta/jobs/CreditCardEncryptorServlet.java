 ...
 */
public class CreditCardEncryptorServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private static  boolean isJobStarted;
	private static String PROPERTY_FILE_NAME = "TPNRCCIx.txt";
...
//	static BufferedWriter bufferedWriter = null;
	 ...
	 */
	public CreditCardEncryptorServlet() {
		super();
	}
...
//				}else 
				if(request.getRemoteHost().equals("mspj8103148.delta.rl.delta.com")){
					...
					Logger.log(CreditCardEncryptorServlet.class, Logger.LEVEL_ERROR, "Endpoint is null for the host IP:"+request.getRemoteHost());
				...
				while ((line = input.readLine()) != null) {
					...
					if(count ==1000){
						...
						CreditCardEncryptorServlet.getEncryptedCC(list,"SPL",endpointUrl);
						count =0;
					}			
					records = records+1;
					System.out.println("Records Written to file is: "+records);
				}
				if(count>0){
					CreditCardEncryptorServlet.getEncryptedCC(list,"SPL",endpointUrl);
...
public static void getEncryptedCC(List<String> creditCards, String userId, String endpointUrl) throws Exception{
		...
		if(serviceResponse !=null){
			if(serviceResponse.getLookupArrayOfTokensResult() != null && serviceResponse.getLookupArrayOfTokensResult().getStatusList() != null 
				...
				Logger.log(CreditCardEncryptorServlet.class, Logger.LEVEL_INFO, serviceResponse.getLookupArrayOfTokensResult().getRawTraceText());
